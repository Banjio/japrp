...
class JsonView(QtWidgets.QWidget):
    ...
    def __init__(self, *, fpath=None, jdata=None):
        ...
        self.found_idx = 0
        if (fpath is None and jdata is None) or (fpath is not None and jdata is not None):
            raise ValueError("Of jdata and fpath exactly one must be specified")
        if jdata is None:
            jfile = open(fpath)
...
class JsonViewer(QtWidgets.QDialog):
    ...
    def __init__(self, jdata=None, fpath=None):
        super(JsonViewer, self).__init__()

        json_view = JsonView(fpath=fpath, jdata=jdata)
...
def main():
    ...
    dict_ = {"A": "1", "B": {"C": "2", "D": "3"}}
    json_viewer = JsonViewer(jdata=dict_)
